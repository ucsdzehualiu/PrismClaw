...
def main():
    ...
    log_ok(f"Python {py_ver.major}.{py_ver.minor}.{py_ver.micro} ({platform.system()} {platform.machine()}) — 满足最低要求 3.7+")

    # 可选依赖 certifi：未安装不阻断，仅提示
    try:
        import certifi  # noqa: F401
        log_ok("可选依赖 certifi 已安装（HTTPS 证书验证已强化）")
    except ImportError:
        log_info("  [--] 可选依赖 certifi 未安装，将使用系统 CA（一般无需处理；如需强化可执行 pip install certifi）")
