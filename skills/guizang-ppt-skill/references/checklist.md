...
做本地验收时，建议给 deck 加 `?slide=N` 单页跳转，方便直接打开关键页截图或检查溢出。注意模板里的 `go()` 会设置短暂 `lock`，所以不要写成 `go(0); setTimeout(()=>go(n), 80)`，第二次跳转可能被锁吞掉。应在首次 `go()` 前算好 `initialSlide` 后只调用一次 `go(initialSlide)`。

浏览器无障碍 snapshot 可能会列出横向 deck 的离屏页面文本，这不等于当前页显示错。确认当前可见页时优先看：底部 active dot、`#deck` 的 `transform`、`document.elementFromPoint()`、截图或视觉检查。
