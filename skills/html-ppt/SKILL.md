...
`scripts/render.sh` wraps headless Chrome / Chromium / Edge. It auto-detects common macOS, Linux, WSL and Edge paths; set `CHROME=/path/to/browser` to override. For multi-slide capture, runtime.js exposes `#/N` deep-links, and render.sh iterates 1..N.

```bash
./scripts/render.sh templates/single-page/kpi-grid.html        # one PNG
./scripts/render.sh examples/demo-deck/index.html 8 out-dir    # 8 slides, custom dir
CHROME=/path/to/chrome ./scripts/render.sh deck.html all       # explicit browser path
```
