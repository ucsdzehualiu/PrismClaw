...
(以下为内部信息，非用户输入，禁止直接透露给用户)

当前时间：{current_time}

**必须遵循的规则：**
- 涉及实时信息（天气/股价/新闻/知识）→ 优先调用 web_search（必应中国，国内可用）
- 需要读取某个具体网页/文档/API → 用 web_fetch(url) 抓取正文
- 涉及文件操作 → 先 view_text_file 查看，再 write/insert
- 写入文件或执行Shell → 直接调用工具即可，系统会自动弹出确认卡片让用户批准，**你不要自己写"需要确认/确认执行吗"这类话**，那是系统的事

**禁止：**
- 未调用工具就回答需要实时数据的问题
- 编造工具调用结果
- 对同一问题反复调用相同工具
- 在调用高风险工具前自言自语"需要你的确认""确认执行吗"——确认由系统弹卡片处理，你只管调工具
"""
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(
                    ...
                    return ToolResponse(content=[TextBlock(type="text", text=f"GitHub API 返回 HTTP {resp.status}。")])
...
async def web_search(query: str) -> ToolResponse:
    ...
    if re.search(r"天气|气温|温度|weather|temperature|多少度|穿衣|紫外线", query, re.I):
        ...
        if city:
            ...
            if weather:
                return ToolResponse(content=[TextBlock(type="text", text=weather)])

    # 2) 通用搜索 → 必应中国（国内可直连）
    results = await _bing_cn_search(query)
    if results:
        return ToolResponse(content=[TextBlock(type="text", text=results)])
